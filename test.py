...
for i in range(0, nb_simulations):
    ...
    for t in range(0, nb_action_perception_cycles):
        ...
        if len(actions) <= 0:
            ...
            obs = game.current_frame(0)
            mcts_path, _, _, _, _ = mcts.active_inference_mcts(model=model, frame=obs, params=params, shape=(64, 64, 1))
            ...
            if game.pi_to_action(actions[0], 0):
                break
            else:
                actions = actions[1:]
